...
public class Network {
	...
	public static void ForwardInference(MolecularNode n, Context ctxt) {
		LinkedList<DownCable> dcs = getValues(n.getDownCableSet()
				.getDownCables());
		for (int i = 0; i < dcs.size(); i++) {
			Relation r = dcs.get(i).getRelation();
			LinkedList<InferenceTuple> FwdPaths = r.getFwdPaths();
			for (int j = 0; j < FwdPaths.size(); j++) {
				InferenceTuple it = FwdPaths.get(j);
				if (it.getFlags().get(j).isStatus()) {
					for (int k = 0; k < it.getPtails().size(); k++) {
						LinkedList<Object[]> st = it.getPtails().get(k)
								.follow(n, new PathTrace(), ctxt);
						LinkedList<Object[]> sh = it.getPheads().get(k)
								.follow(n, new PathTrace(), ctxt);
						for (int z = 0; z < st.size(); z++) {
							LinkedList<Object[]> sn = it
									.getRelation()
									.getPath()
									.follow((Node) st.get(z)[0],
											new PathTrace(), ctxt);
							LinkedList<Object[]> si = Intersection(sh, sn);
							LinkedList<NodePair> c = CrossProduct(si, st);
							for (int y = 0; y < c.size(); y++) {
								MolecularNode n1 = (MolecularNode) c.get(y)
										.getA1();
								Node n2 = c.get(y).getA2();
								InfereRelation(n1, n2, r);
							}
						}
					...
					for (int k = 0; k < it.getPtails().size(); k++) {
						...
						LinkedList<Object[]> sh = it.getPheads().get(k)
								.follow(n, new PathTrace(), ctxt);
						LinkedList<NodePair> c = CrossProduct(st, sh);
						for (int z = 0; z < c.size(); z++) {
							MolecularNode n1 = (MolecularNode) c.get(z).getA1();
							Node n2 = c.get(z).getA2();
							InfereRelation(n1, n2, r);
						}
	...
	public static void ForwardInference(MolecularNode n) {
		LinkedList<DownCable> dcs = getValues(n.getDownCableSet()
				.getDownCables());
		for (int i = 0; i < dcs.size(); i++) {
			Relation r = dcs.get(i).getRelation();
			...
			for (int j = 0; j < FwdPaths.size(); j++) {
				...
				if (it.getFlags().get(j).isStatus()) {
					for (int k = 0; k < it.getPtails().size(); k++) {
						LinkedList<Object[]> st = it
								.getPtails()
								.get(k)
								.follow(n, new PathTrace(),
										SNeBR.SNeBR.getCurrentContext());
						LinkedList<Object[]> sh = it
								.getPheads()
								.get(k)
								.follow(n, new PathTrace(),
										SNeBR.SNeBR.getCurrentContext());
	...
	public static LinkedList<NodePair> CrossProduct(LinkedList<Object[]> l1,
			LinkedList<Object[]> l2) {
		LinkedList<NodePair> result = new LinkedList<NodePair>();
		for (int i = 0; i < l1.size(); i++) {
			for (int j = 0; j < l2.size(); j++) {
				NodePair np = new NodePair(l1.get(i), l2.get(j));
				result.add(np);
			}
		}
		return result;
	...
	public static LinkedList<Object[]> Intersection(LinkedList<Object[]> l1,
			LinkedList<Object[]> l2) {
	...
	public static void InfereRelation(MolecularNode n1, Node n2, Relation r) {

		Hashtable<String, DownCable> dcset = n1.getDownCableSet()
				.getDownCables();
		LinkedList<DownCable> dcs = getValues(dcset);
		LinkedList<Relation> NodeRelations = new LinkedList<Relation>();
		for (int y = 0; y < dcs.size(); y++) {
			if (dcs.get(y).getRelation().getAdjust() == "reduce"
					&& dcs.get(y).getRelation().getLimit() == 0) {

			} else {
				NodeRelations.add(dcs.get(y).getRelation());
			}
		}
		LinkedList<CaseFrame> cfs = new LinkedList<CaseFrame>();
		cfs.addAll(Network.getCaseFrames().values());
		LinkedList<CaseFrame> ValidCf = new LinkedList<CaseFrame>();
		for (int i = 0; i < cfs.size(); i++) {
			if (cfs.get(i).getRelations().contains(r)) {
				ValidCf.add(cfs.get(i));
			}
		}

		if (!ValidCf.isEmpty()) {
			for (int j = 0; j < ValidCf.size(); j++) {
				CaseFrame cf = ValidCf.get(j);
				LinkedList<RCFP> rcfps = new LinkedList<RCFP>();
				rcfps.addAll(cf.getRelations().values());
				LinkedList<Relation> relations = new LinkedList<Relation>();
				for (int z = 0; z < rcfps.size(); z++) {
					relations.add(rcfps.get(z).getRelation());
				}
				LinkedList<Relation> intersection = new LinkedList<Relation>();
				LinkedList<Relation> difference = new LinkedList<Relation>();
				for (int m = 0; m < relations.size(); m++) {
					if (NodeRelations.contains(relations.get(m))) {
						intersection.add(relations.get(m));
					} else {
						difference.add(relations.get(m));
					}
				}
				if (difference.isEmpty()) {
					System.out.println("should build n1");
				}

			}
		} else {
			System.out.println("Cannot build inferred node");

		}
	}

	public static LinkedList<DownCable> getValues(Hashtable<String, DownCable> h) {

		LinkedList<DownCable> dcs = new LinkedList<DownCable>();
	...
	/**
	 * This method checks whether the down cable set of a certain molecular node
	 * is following the restrictions specified by the case frame or not.(exactly
	...
	private static boolean followingCaseFrame(Object[][] array,
		...
		Hashtable<String, RCFP> list = new Hashtable<String, RCFP>(
				caseFrame.getRelations());
		for (int i = 0; i < array.length; i++) {
			Relation r = (Relation) array[i][0];
			if (list.containsKey(r.getName())) {
				if (((NodeSet) array[i][1]).size() >= caseFrame.getRelation(r)
						.getLimit()) {
	...
	// //////////////////////////////////// the main method that was used for
	// testing //////////////////////////////////////////
...
*/
	 ...
	 * public static void main(String[] args) throws Exception {
	 * 
	 * Relation member = new Relation("member", "Entity", "reduce", 1); Relation
	 * sub = new Relation("sub", "Entity", "reduce", 1); Relation sup = new
	 * Relation("super", "Entity", "reduce", 1); VariableNode node1 = new
	 * VariableNode("Entity", "x"); Path p = new FUnitPath(member); Path p1 =
	 * new BUnitPath(sup); Path p2 = new FUnitPath(sub); LinkedList<Path> paths
	 * = new LinkedList<Path>(); paths.add(p1); paths.add(p2); paths.add(p);
	 * AndPath ap = new AndPath(p1, p2, p); DomainRestrictPath drp = new
	 * DomainRestrictPath(p, node1, p2); OrPath op = new OrPath(paths);
	 * RangeRestrictPath rsp = new RangeRestrictPath(p1, p2, node1);
	 * LinkedList<Path> paths1 = new LinkedList<Path>(); paths1.add(ap);
	 * paths1.add(drp); paths1.add(rsp); paths1.add(op); // member.setPath(new
	 * ComposePath(paths1));
	 * 
	 * LinkedList<CompilationTuple> cts = ap.compile(); for (int i = 0; i <
	 * cts.size(); i++) { System.out.println(i);
	 * System.out.println(cts.get(i).getPhead().toString());
	 * System.out.println(cts.get(i).getPtail().toString());
	 * System.out.println(">>>>>>>>>>>>>>>>");
	 * 
	 * }
	 * 
