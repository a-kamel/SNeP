...
public class ComposePath extends Path {
	...
	/**
	 * a LinkedList of paths included in this composed path. The paths included
	 * in the composed path can be of any type.
	 */
	...
	/**
	 * 
	 * @return the list of paths included in the current composed path.
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see sneps.Paths.Path#follow(sneps.Nodes.Node, sneps.PathTrace,
	 *      SNeBR.Context)
	 */
	@Override
	...
	/**
	 * A helper follow method that is invoked from the original follow method.
	 * IT follows the given path starting at all nodes in the given list of
	 * pairs.
	 * 
	 * @param temp
	 *            a LinkedList of Node-PathTrace pairs.
	 * @param context
	 *            the context that the propositions along this path are asserted
	 *            in.
	 * @param path
	 *            the path that will be followed starting at nodes in the list
	 *            of pairs.
	 * 
	 * @return a LinkedList of Node-PathTrace pairs resulted from following the
	 *         given path starting at all nodes in the given list of pairs.
	 */
	private LinkedList<Object[]> follow(LinkedList<Object[]> temp,
			Context context, ComposePath path) {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see sneps.Paths.Path#followConverse(sneps.Nodes.Node, sneps.PathTrace,
	 *      SNeBR.Context)
	 */
	@Override
	...
	/**
	 * A helper follow method that is invoked from the original follow method.
	 * It follows the converse of the given path starting at all nodes in the
	 * given list of pairs
	 * 
	 * @param temp
	 *            a LinkedList of Node-PathTrace pairs.
	 ...
	 *            in.
	 * @param path
	 *            the path that its converse will be followed starting at nodes
	 *            in the list of pairs.
	 * 
	 * @return a LinkedList of Node-PathTrace pairs resulted from following the
	 *         converse of the given path starting at all nodes in the given
	 *         list of pairs.
	 */
	private LinkedList<Object[]> followConverse(LinkedList<Object[]> temp,
			Context context, ComposePath path) {
		if (path.getPaths().isEmpty())
			return temp;
		LinkedList<Object[]> result = new LinkedList<Object[]>();
	 ...
	 * (non-Javadoc)
	 * 
	 * @see sneps.Paths.Path#clone(sneps.Nodes.Node, sneps.PathTrace,
	 *      SNeBR.Context)
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * 
	 * @see sneps.Paths.Path#equals(sneps.Nodes.Node, sneps.PathTrace,
	 *      SNeBR.Context)
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * 
	 * @see sneps.Paths.Path#converse(sneps.Nodes.Node, sneps.PathTrace,
	 *      SNeBR.Context)
	 */
	@Override
	...
	@Override
	public LinkedList<CompilationTuple> compile() {

		LinkedList<Path> left = new LinkedList<Path>();
		LinkedList<Path> right = new LinkedList<Path>(paths);
		LinkedList<CompilationTuple> result = new LinkedList<CompilationTuple>();
		while (!paths.isEmpty()) {
			LinkedList<CompilationTuple> l = paths.removeFirst().compile();
			for (int i = 0; i < l.size(); i++) {
				Path ph = l.get(i).getPhead();
				LinkedList<Path> r = new LinkedList<Path>();
				LinkedList<Path> lef = new LinkedList<Path>();
				...
				ComposePath ph1 = compose(ph, r);
				l.get(i).setPhead(ph1);
				Path pt = l.get(i).getPtail();
				ComposePath pt1 = compose(pt, lef);
				l.get(i).setPtail(pt1);
				result.add(l.get(i));
