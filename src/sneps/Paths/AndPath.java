...
public class AndPath extends Path {
	...
	/**
	 * A LinkedList of Paths included in this and-path. The paths included in
	 * the and-path can be of any type.
	 */
	...
	public LinkedList<Object[]> follow(Node node, PathTrace trace,
		...
		if (pList.size() > 0)
			return intersectionAnd(p.follow(node, trace, context),
					andPath.follow(node, trace, context));
		else
	...
	public LinkedList<Object[]> followConverse(Node node, PathTrace trace,
		...
		AndPath andPath = new AndPath(pList);
		if (pList.size() > 0)
			return intersectionAnd(p.followConverse(node, trace, context),
					andPath.followConverse(node, trace, context));
		else
	 ...
	 *         intersection between the two given lists
	 */
	private LinkedList<Object[]> intersectionAnd(LinkedList<Object[]> l1,
	...
	public LinkedList<CompilationTuple> compile() {
		...
		for (int j = 0; j < paths.size(); j++) {
			LinkedList<CompilationTuple> ct = paths.get(j).compile();
			for (int i = 0; i < ct.size(); i++) {
				LinkedList<Path> t = new LinkedList<Path>();
				t.add(paths.get(j));
				LinkedList<Path> t2 = difference(paths, t);
				Path p = new AndPath(t2).converse();
				Path ph = ct.get(i).getPhead();
				ct.get(i).setPtail(new ComposePath(ph, p));
				ct.get(i).setFlag(true);
				result.add(ct.get(j));
			}
