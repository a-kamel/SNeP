...
public class KPlusPath extends Path {
	...
	/**
	 * a path that is to be followed one or more times until no more new nodes
	 * can be reached by following it again. This path can be of any type.
	 */
	private Path path;
	 ...
	 * 
	 * @param path
	 *            the path defined in the k-plus path.
	 */
	...
	/**
	 * 
	 * @return the path specified in the current k-plus path.
	 */
	public Path getPath() {
		return this.path;
	}
	...
	/**
	 * A helper follow method. It follows the current path starting at the nodes
	 ...
	 *         path starting at all nodes in temp until no more nodes are
	 *         reachable.
	 */
	...
	public LinkedList<Object[]> followConverse(Node node, PathTrace trace,
		...
		LinkedList<Object[]> temp = this.path.followConverse(node, trace,
				context);
	...
	/**
	 * A helper followConverse method. It follows the converse of the current
	 * path starting at nodes in the list of pairs (temp) in the given context.
	 *
	 * @param temp
	 *            a LinkedList of Node-PathTrace pairs.
	 * @param context
	 ...
	 * 
	 * @return a LinkedList of Node-PathTrace pairs resulted from following the
	 *         converse of the path starting at all nodes in temp until no more
	 *         nodes are reachable.
	 */
	private LinkedList<Object[]> followConverse(LinkedList<Object[]> temp,
		...
		for (int i = 0; i < temp.size(); i++) {
			...
			Node node = (Node) o[0];
			PathTrace trace = (PathTrace) o[1];
			LinkedList<Object[]> f = this.path.followConverse(node, trace,
					context);
			result.addAll(f);
		}
		while (!result.isEmpty()) {
	...
	public LinkedList<CompilationTuple> compile() {
		...
		LinkedList<CompilationTuple> l = this.path.compile();
		for (int i = 0; i < l.size(); i++) {
			Path ph = l.get(i).getPhead();
			Path pt = l.get(i).getPtail();
			ComposePath p = new ComposePath(pt, this.converse());
			ComposePath p1 = new ComposePath(ph, this);
			l.get(i).setPtail(p);
			l.get(i).setPhead(p1);
			result.add(l.get(i));
		}
