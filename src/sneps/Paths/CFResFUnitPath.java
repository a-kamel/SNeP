...
public class CFResFUnitPath extends Path {
	...
	/**
	 * The relation that labels the arc of this case frame restricted forward
	 * unit path.
	 */
	private Relation relation;

	/**
	 * The case frame that this case frame restricted forward unit path is
	 * restricted to.
	 */
	...
	/**
	 * 
	 * @return the case frame that the current path is restricted to.
	 */
			...
			if (dSet.getCaseFrame().getId().equals(this.caseFrame.getId())) {
				...
				if (dCable != null) {
					...
					for (int i = 0; i < ns.size(); i++) {
						...
						t.compose(new CFResFUnitPath(this.relation,
								this.caseFrame));
	...
	public LinkedList<Object[]> followConverse(Node node, PathTrace trace,
			Context context) {
		return new CFResBUnitPath(this.relation, this.caseFrame).follow(node,
				trace, context);
	}
	...
	public boolean equals(Object obj) {
		...
		if (!((CFResFUnitPath) obj).getRelation().equals(this.relation))
			return false;
		if (!((CFResFUnitPath) obj).getCaseFrame().getId()
				.equals(this.caseFrame.getId()))
			return false;
		return true;
	}
	...
	public LinkedList<CompilationTuple> compile() {
		CompilationTuple ct = new CompilationTuple(this.getRelation(), this,
				new EmptyPath(), false);
		LinkedList<CompilationTuple> cts = new LinkedList<CompilationTuple>();
		cts.add(ct);
		return cts;
	}
