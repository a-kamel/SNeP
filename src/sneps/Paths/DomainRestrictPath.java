...
public class DomainRestrictPath extends Path {
	...
	/**
	 * A path that should exist between the starting node and the node that is
	 * specified in this domain restrict path. This path can be of any type.
	 */
	private Path q;

	/**
	 * A node specified in the domain restrict path that should be reachable if
	 * path Q is to be followed from the starting node.
	 */
	private Node zNode;

	/**
	 * A path that should be followed from the starting node to get all the
	 * nodes reachable by this path, given that path Q is path from the starting
	 * node to the node specified in this domain restrict path. This path can be
	 * of any type.
	 */
	private Path p;
	...
	public LinkedList<Object[]> followConverse(Node node, PathTrace trace,
			Context context) {
		return new RangeRestrictPath(new ConversePath(this.p), this.q,
	...
	public DomainRestrictPath clone() {
		return new DomainRestrictPath(this.q.clone(), this.zNode,
				this.p.clone());
	}
	...
	public String toString() {
		return "Domain-restrict(" + this.q.toString() + ", "
	...
	public LinkedList<CompilationTuple> compile() {
		LinkedList<CompilationTuple> result = this.getP().compile();
		LinkedList<CompilationTuple> temp = this.getQ().compile();
		for (int i = 0; i < result.size(); i++) {
			result.get(i).setPtail(
					new RangeRestrictPath(result.get(i).getPtail(),
							this.getQ(), this.getZNode()));
			result.get(i).setFlag(false);
		}
		for (int i = 0; i < temp.size(); i++) {
			temp.get(i).setPhead(
					new DomainRestrictPath(temp.get(i).getPhead(), this
							.getZNode(), new ComposePath(
							temp.get(i).getPtail(), this.getP())));
			temp.get(i).setFlag(false);
