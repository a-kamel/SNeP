...
public class RangeRestrictPath extends Path {
	...
	/**
	 * a Path that should be followed to get all the reachable nodes that have
	 * path Q as path from them to the node N specified in this range restrict
	 * path. This path can be of any type.
	 */
	private Path p;

	/**
	 * A path that should exist between the node, that is reachable by following
	 * path P from the starting node, and the node N specified in this range
	 * restrict path. This path can be of any type.
	 */
	private Path q;
	...
	public LinkedList<Object[]> followConverse(Node node, PathTrace trace,
			Context context) {
		return new DomainRestrictPath(this.q, this.zNode, new ConversePath(
				this.p)).follow(node, trace, context);
	}
	...
	public String toString() {
		return "range-restrict(" + this.p.toString() + ", " + this.q.toString()
				+ ", " + this.zNode.toString() + ")";
	...
	public LinkedList<CompilationTuple> compile() {
		...
		for (int i = 0; i < result.size(); i++) {
			result.get(i).setPtail(
					new RangeRestrictPath(result.get(i).getPhead(),
							this.getQ(), this.getNode()));
			result.get(i).setFlag(false);
		}
		for (int i = 0; i < temp.size(); i++) {
			temp.get(i).setPhead(
					new DomainRestrictPath(temp.get(i).getPhead(), this
							.getNode(), temp.get(i).getPtail()));
			temp.get(i).setPtail(
					new ComposePath(temp.get(i).getPtail(), this.getP()
							.converse()));
