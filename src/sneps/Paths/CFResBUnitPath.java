...
public class CFResBUnitPath extends Path {
	...
	/**
	 * The relation that labels the arc of this case frame restricted backward
	 * unit path.
	 */
	private Relation relation;

	/**
	 * The case frame that this case frame restricted backward unit path is
	 * restricted to.
	 */
	...
	public LinkedList<Object[]> followConverse(Node node, PathTrace trace,
			Context context) {
		return new CFResFUnitPath(this.relation, this.caseFrame).follow(node,
				trace, context);
	}
	...
	public boolean equals(Object obj) {
		...
		if (!((CFResBUnitPath) obj).getRelation().equals(this.relation))
			return false;
		if (!((CFResBUnitPath) obj).getCaseFrame().getId()
	...
	public LinkedList<CompilationTuple> compile() {
		CompilationTuple ct = new CompilationTuple(this.getRelation(), new EmptyPath(),
				this, false);
